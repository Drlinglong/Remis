...
def test_build_file_task_iterator_removes_reference_hits_from_model_batch(monkeypatch):
    ...
    class Resolver:
        def lookup(self, key, source_text, source_file=""):
            if key == "TRK_ADJ" and source_text == "Turkana":
                return Match("图尔卡纳")
            return Match()
    ...
    assert len(task.semantic_hints) == len(task.texts_to_translate) == 1
    assert "country_adjective_definition" in task.semantic_hints[0]
    assert protected_entries == [{"source_file": "countries.yml", "key": "TRK_ADJ"}]
