...
def test_analysis_run_benchmark_scores_persisted_aggregation_assignments(tmp_path: Path):
    database = tmp_path / "context.sqlite"
    _create_analysis_run_database(database)
    gold = tmp_path / "gold.md"
    gold.write_text(
        "\n".join(
            [
                "| Unit | Keys | Chain | Relation | Note | Confidence |",
                "|---|---|---|---|---|---|",
                "| `unit_0` | `event.1` | `gold_arc` | `primary_member` | first | `high` |",
                "| `unit_1` | `modifier.1` | `gold_arc` | `supporting_context` | second | `high` |",
            ]
        ),
        encoding="utf-8",
    )
    ...
    assert snapshot["target"]["kind"] == "analysis_run"
    assert snapshot["target"]["id"] == "run-1"
    assert snapshot["target"]["source_snapshot_hash"] == "snapshot-1"
    assert snapshot["metrics"]["delivery_f1"] == 1.0
    assert snapshot["predicted_to_gold_chain_mapping"] == {
        "predicted_arc": "gold_arc"
    }
    assert snapshot["units"][1]["predicted_links"][0]["relation"] == (
        "supporting_context"
    )


def _create_analysis_run_database(path: Path) -> None:
    import json
    import sqlite3
    ...
    source_rows = [
        (
            "source-1",
            "project-1",
            "fixture.yml::0:event.1",
            "Event text",
            json.dumps(
                {
                    "relative_path": "fixture.yml",
                    "item_key": "event.1",
                    "source_order": 0,
    ...
    connection.executemany(
        "INSERT INTO context_source_items VALUES (?, ?, ?, ?, ?)", source_rows
    )
    assignments = [
        {
    ...
    connection.execute(
        ...
        (
            ...
            "aggregation",
            1,
            json.dumps({"assignment_batch": {"assignments": assignments}}),
            "succeeded",
        ),
    )
    connection.commit()
