...
Project Zomboid 与 RimWorld 在 3.2.1 中仍为 **Preview**。RimWorld 的完整版本号（例如 `1.6.4512`）会匹配 `LoadFolders.xml` 中的 `v1.6` 分支。遇到 `IfModActive` 等条件且没有明确的已启用 Mod 集合时，资源发现会阻止翻译，不会让不确定的兼容层覆盖基础文本；当前普通流程不会自动推断玩家启用了哪些 Mod，应保留诊断并人工核对。
