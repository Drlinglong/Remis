...
> **说明（请先读）**
> 本文是 **原理向导览**：解释 Remis 在背后如何拆文件、用词典、组提示词、再写出汉化包。
> **不是**「第一次打开客户端该点哪里」的操作手册。
>
> - 要从零汉化：请读 **[从零开始：第一次汉化](getting-started.md)**（**项目管理 → 创建新项目 → 初次翻译 → 部署**）。
> - Mod 更新了：请读 **[增量翻译](incremental-update.md)**。
> - 半成品译文：请读 **[导入已有译文](import-existing-translations.md)**。
> - 进游戏仍英文：请读 **[假本地化](fake-localization.md)**。
...
所以，工厂里最核心的工序之一，就是给AI精心准备一张“小纸条”，也就是**指令（Prompt）**。这张“小纸条”会根据游戏、语言和当前条目动态组装，主要包含以下部分：
...
6.  **按需加入的语义提示 (Semantic Context)**
    某些游戏条目只有结合运行方式才能正确翻译。例如 Victoria 3 的官方国家 `TAG_ADJ` 既有定义，也会作为变量插入别的句子。Remis 会在本地识别这类条目，只向 AI 补充当前目标语言所需的最小语义规则，不把整个 Key 列表或其他条目的答案一股脑交给模型。引用句仍必须保留变量，助词、词序或词形由引用位置根据目标语言处理；难以自动确认的结果会留给人工复核。
