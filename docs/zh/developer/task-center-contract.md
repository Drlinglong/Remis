...
当前 `allowed_actions` 按任务类型和状态提供：

- 活动任务：`view_task`；支持协作取消的翻译任务另外提供 `cancel_task`
- 兼容检查点的中断翻译：`view_task`、`resume_task`、`start_over_task`、`archive_task`
- 其它失败／中断：`view_task`、`return_to_workflow`、`archive_task`
- 完成／取消：`view_task`、`archive_task`
- 已处理：`view_task`、`restore_task`

3.2.0 仍没有不分任务类型的通用重试、删除或整次重跑 API；翻译取消、继续和从头开始由
所属恢复／任务路由重新校验参数、检查点、锁和批准。Task Center 不删除历史记录。
...
3.2.0 的 Agent/Copilot 已在 stable 与 Agent Preview 发布通道开放；Agent 仍必须使用
capabilities、preflight 和所属业务流程的确认门，不能因为有读取权限就获得写入权限。
...
| 重试、取消、删除、重跑必须确认 | 翻译取消／继续／从头开始有任务类型门禁；无通用删除 | 取消与恢复已实现；删除仍是明确非目标 |
| 最新任务优先 | repository 与抽屉按最近活动倒序 | 一致 |
| 主页不维护第二套任务状态 | “正在进行的工作”复用 Context | 一致 |
| “近期活动”只表达项目变化 | 数据确为项目历史，但名称和部分事件与任务近似 | 信息架构待收敛 |
| Agent 随时知道项目发生什么 | 已有结构化查询基础和已发布档案读取；复杂编排仍未开放 | 部分一致 |
