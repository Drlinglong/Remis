...
不包含主动语义扫描、项目级文风校对、术语管理、整文件重翻、key 修复或部署。格式规则
命中的条目进入模型后，允许对该条完整句子做有限的源文感知修订。
...
`ReflexionFixAgent._build_batch_prompt()` 允许三类模式：

- format repair；
- failed-chunk recovery；
- limited source-aware revision。

允许纠正明显误译、极性、强度、遗漏和生硬表达符合产品决定。真正的缺口是工坊 prompt
没有收到本次翻译使用的项目词典、手选词典和全局词典上下文。当模型审视完整句子时，可能
把原本正确的专有名词改成另一种译法。

**待共同确认**：核验翻译时实际使用的词典快照如何持久化，并决定内嵌／独立工坊加载哪一
...
8. 只有格式规则命中的条目可以进入；允许该条有限语义恢复，但禁止自由润色和跨条修改。
