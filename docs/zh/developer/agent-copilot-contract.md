...
- stable 与 Agent Preview 发布通道均启用 `FEATURES.ENABLE_REMIS_COPILOT`；
...
- stable 与 Agent Preview 的 frozen 打包态均注册 `/api/copilot`；
- `/api/agent` 始终是另一套本机 API，不等同于公开 Copilot。

版本与通道由 `scripts/build_profile.py` 统一定义。未知通道对档案馆和断点续传功能采取
fail-closed 策略；前后端不能只显示按钮而让打包后端返回 404。
...
- 删除原始 Mod 数据、词典、项目等高风险步骤：在计划中醒目标注对象、范围和后果，由
  整份计划批准涵盖，不增加节点级确认；
...
`deploy_mod`。Remis 也没有独立的“删除部署数据”能力；同名部署覆盖是部署步骤，不是
删除 action。文档中的未来候选不得被模型当作现有能力。

手动进入某个功能时，现有模块 UI 可以继续使用自己的确认界面。Agent 执行则应把整份计划
批准转换成各业务服务可验证的、限定 action 和范围的 approval，不应要求用户再次点击。
...
3.2.0 尚无此通用 DAG。当前聊天工作流仍主要支持硬编码的创建项目 + 初次翻译组合；
Task Center 的任务恢复、取消、档案馆发布和 Agent API 能力属于各自受治理的业务入口，
不是任意多步骤聊天编排。

## 当前主要差距

1. 聊天仍没有通用 DAG；增量翻译、更新检查、新词挖掘和封面生成不能被模型任意串联。
2. 聊天工作流计划保存在内存中，应用重启后必须重新规划，不能复用旧批准。
3. 聊天入口主要报告任务已启动；最终完成、部分成功和失败仍以 Task Center 与业务结果为准。
4. 文档中列出的未来候选 action 不能当作当前 Registry；当前可提议名称必须以
   `/api/copilot/actions` 返回值为准。
5. Agent 的 context-analysis 直接计划/启动接口尚未开放；档案馆分析请走 Remis 原有工作流。
6. repair、export 和删除档案数据均有独立风险边界；执行前必须提供对应批准和对象校验。
7. 复杂档案分析的实体归并、事件链连续性和来源污染仍需结合真实语料人工抽查，不能只看任务成功。
...
16. 高风险删除在计划中醒目标注并由整份批准涵盖，不触发节点级二次确认。
17. “删除部署数据”作为未知 action 被拒绝；同名覆盖只能作为部署步骤执行。

18. stable 与 Agent Preview 的版本、端口、数据目录和 Copilot 开关必须保持通道隔离；
    未知构建通道必须关闭档案馆与断点续传。
