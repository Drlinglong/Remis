...
1. 中文和英文部分都必须以 `主要更新` / `Highlights` 开头。
2. 该区域只放用户真正可能在意的重要变化：重要新功能、现有流程或 UI／UX／HCI
   的重大变化、使用方式的重大变化，或者可能影响已有项目并需要用户操作的变化。
3. `主要更新` 必须让非开发者无需理解实现术语也能读懂。应描述使用效果，例如
   “工坊预览可以准确还原链接、列表和分隔线”，而不是解释解析器内部逻辑。
4. 重构、文件名、函数名、Issue、内部架构、测试命令、安全实现等技术内容放到下方
   的 `工程质量与可靠性` / `Engineering quality and reliability`。
