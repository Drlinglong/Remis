...
程序完成初步归并和覆盖计数后，将候选清单、项目摘要和原文证据发送给模型。
完整档案模式默认执行 A/B 实体摘要；仅术语模式跳过。该调用负责：

- 对 A/B 实体及其可能别名做语义去重；
- 为最终 A/B 实体生成摘要；
- 返回所使用的 evidence unit IDs。

同一规范实体在不同 #2 分块中的证据必须先全部归拢，并优先放进同一个摘要
请求，使“门徒”与“后来背叛”等跨分块信息能够共同参与总结。每个请求片段
默认最多容纳 12 个 local units、8,000 个原文字符；该上限只控制请求切片，
不得永久排除 A/B 实体的其余证据。

若单个实体超过一个请求片段，程序按原始顺序和事件组边界确定性拆分：每个
片段先产生局部实体摘要，再将所有局部摘要与项目摘要送入一次实体级合并，
形成最终摘要。一个请求可批量容纳多个实体，但应优先保持同一实体的全部
证据在同一批；只有单个实体自身超过预算时才拆分。C 级候选只以紧凑名称/
局部描述参与别名判断，不生成长摘要。最终等级在语义合并后由程序重新计算。

#2 在每个分块中返回的实体描述、原文证据、批次来源和别名判断必须全部保留。
高级界面展示完整分块证据、其 `digest_segment_id` 以及是否进入最终摘要；用户
应能对比机械拼接的局部描述、分段摘要与最终 LLM 摘要。这样后续评估可以决定
保留摘要调用，还是改为纯程序拼接，而不需要重新运行 #2。
...
实体高级详情必须展示全部分块证据、摘要分段归属、机械拼接文本、可选分段
摘要与最终摘要。默认普通用户仍只看最终 A/B 摘要和简明覆盖信息。
