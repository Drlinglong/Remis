...
- 默认使用中文交流。
- 强制使用英文编写 Git Commit 消息，严格遵循 `<type>(<scope>): <subject>` 规范，不再使用中文提交。
...
- 强制语言包（Locale）防污染双校验纪律：任何 locale 文件的修改必须在提交前进行 `JSON.parse` 格式校验和全文搜索连续的问号污染。提交前必须运行并通过 `textEncodingIntegrity.test.js` 编码完整性测试。
