...
import JudgmentCourtCardView from '../components/neologism/JudgmentCourtCardView';
import JudgmentCourtViewToolbar from '../components/neologism/JudgmentCourtViewToolbar';
...
import { useJudgmentCourtPresentation } from '../components/neologism/useJudgmentCourtPresentation';
...
const candidates = [
    ...
    { id: 3, original: 'Quiet Star Watch', suggestion: '寂星守望者', frequency: 2, tier: 'B', context_snippets: ['The Quiet Star Watch records every transmission.'] },
    { id: 4, original: 'Signal Reliquary', suggestion: '信号圣龛', frequency: 2, tier: 'B', context_snippets: ['Open the Signal Reliquary.'] },
    { id: 5, original: 'The Exceptionally Long Unbroken Candidate Identifier For Overflow Verification', suggestion: '用于溢出验证的超长候选术语', frequency: 1, tier: 'C', context_snippets: ['Overflow evidence remains readable.'] },
    { id: 6, original: 'Void Beacon', suggestion: '虚空信标', frequency: 1, tier: 'C', context_snippets: ['The Void Beacon is silent.'] },
];
...
const JudgmentCourtVisualFixture = ({ themeId }) => {
    ...
    const selectedCandidate = presentation.selectedCandidate;
    ...
    return (
        ...
        >
            ...
            <JudgmentCourtViewToolbar
                ...
                onRejectDuplicates={() => {}}
                onSearchQueryChange={presentation.setSearchQuery}
                onTierFilterChange={presentation.setTierFilter}
                onViewModeChange={presentation.setViewMode}
                searchQuery={presentation.searchQuery}
                t={t}
                tierFilter={presentation.tierFilter}
                viewMode={presentation.viewMode}
            />
            {presentation.viewMode === 'cards' ? (
                <JudgmentCourtCardView
                    collapsedTiers={presentation.collapsedTiers}
                    groupedCandidates={presentation.groupedCandidates}
                    onOpenCandidate={presentation.openCandidateInDetail}
                    onToggleTier={presentation.toggleTier}
                    t={t}
                />
            ) : (
                <div
                ...
                >
                    <JudgmentDocket
                        ...
                        candidates={presentation.visibleCandidates}
                        docketView={docketView}
                        focusRequest={presentation.detailFocusRequest}
                        loading={false}
                        onBatchConfirm={() => {}}
                        onSelectCandidate={setSelectedId}
                        onToggleAll={presentation.toggleAllVisibleCandidates}
