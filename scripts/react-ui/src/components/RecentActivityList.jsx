...
import { Paper, Text, Group, Avatar, Stack, Badge, Skeleton } from '@mantine/core';
...
const RecentActivityList = ({
    ...
    return (
        ...
        >
            ...
            <Stack gap="md" data-remis-activity-list>
                {loading ? (
                    Array(5)
                        .fill(0)
                        .map((_, i) => (
                            <Group key={i} wrap="nowrap" style={{ minWidth: 0 }}>
                    ...
                    <Text size="xs" c="dimmed" ta="center" py="xl" style={{ overflowWrap: 'anywhere' }}>
                        {emptyLabel || t('task_center.not_available')}
                    </Text>
                ) : (activities || []).length > 0 ? (
                    activities.slice(0, maxItems).map((activity) => (
                        <Group key={activity.id} align="flex-start" wrap="nowrap" style={{ minWidth: 0 }}>
                            <Avatar color="blue" radius="xl" style={{ flexShrink: 0 }}>
                                {(activity.title || 'SY').substring(0, 2).toUpperCase()}
                            </Avatar>
                            <div style={{ flex: 1, minWidth: 0 }}>
                                <Text size="sm" fw={500} lineClamp={2} title={activity.title} style={{ overflowWrap: 'anywhere' }}>
                                    {activity.title}
                                </Text>
                                <Text c="dimmed" size="xs" lineClamp={2} style={{ overflowWrap: 'anywhere' }}>
                            ...
                            <Badge
                                variant="light"
                                size="xs"
                                style={{ flexShrink: 0, maxWidth: '35%', whiteSpace: 'normal', overflowWrap: 'anywhere' }}
                            >
