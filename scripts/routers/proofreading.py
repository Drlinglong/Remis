...
def get_proofread_data(project_id: str, file_id: str):
    ...
    """
    data = proofreading_service.get_proofread_data(project_id, file_id)
    if not data:
        raise HTTPException(status_code=404, detail="Proofreading data not found")
    return data

@router.post("/api/proofread/save")
def save_proofread_data(request: SaveProofreadingRequest):
    """
    保存校对数据 - Delegation to Service
    """
    success = proofreading_service.save_proofread_data(
        request.project_id, 
        request.file_id, 
