...
class IncrementalPackageService:
    def build_output_folder_name(self, project_name: str, target_lang_info: Dict[str, Any]) -> str:
        ...
        return f"{prefix}{slugify_to_ascii(project_name)}-incremental-update"
    ...
    def prepare_output_package(
        ...
        package_root = Path(OUTPUT_DIR) / output_folder_name
        launcher_mod_path = Path(OUTPUT_DIR) / f"{output_folder_name}.mod"
