...
const chartCopy = {
  en: {
    ...
    intelligenceDesc: 'Blind language preference and hard reliability under versioned Pilot Score recipes · Higher is better',
    recipesMeta: `${pilotRecipes.length} of ${pilotRecipes.length} recipes`, scoreRanking: 'Pilot Quality Score ranking',
    scoreFooter: '60% soft preference · 40% hard reliability · v0.2 challengers use fixed anchors', speedDesc: 'Completed frozen hard samples per wall-clock hour · Higher is better',
    ...
    costRanking: 'Cost per 100 tasks ranking', pricing: 'Google Standard pricing · OpenRouter run-date pricing · local GPU shown separately', judgeExcluded: 'Judge cost excluded',
  ...
  zh: {
    ...
    intelligenceDesc: '版本化 Pilot Score 下的盲裁语言偏好与硬可靠性 · 越高越好',
    recipesMeta: `${pilotRecipes.length} / ${pilotRecipes.length} 套方案`, scoreRanking: 'Pilot Quality Score 排名',
    scoreFooter: '60% 软偏好 · 40% 硬可靠性 · v0.2 新选手采用固定锚点', speedDesc: '每小时完成的冻结硬样本数 · 越高越好',
    ...
    costRanking: '每 100 题成本排名', pricing: 'Google Standard 计价 · OpenRouter 测试当日计价 · 本地 GPU 单列', judgeExcluded: '不含裁判成本',
...
function RankingBars({ recipes, getValue, formatValue, label, labels, lowerIsBetter = false, freeTier = false, onSelect }) {
  ...
  return (
    <div className="aa-ranking-chart" role="list" aria-label={label} style={{ '--recipe-count': recipes.length }}>
      ...
      {recipes.map((recipe) => {
        ...
        const valueLabel = value === null
          ? (recipe.costKind === 'local-hardware' ? labels.localHardware : labels.freeTier)
          : formatValue(value)
          ...
          return (
            <button
              ...
              style={{
                ...
                '--x': `${position.x}%`,
                '--y': `${position.y}%`,
                '--label-shift': `${[-14, 20, 72, 10, -8, 20, 18, 42, -5, -30, 62][index]}px`,
                '--label-rise': `${[0, 18, 44, 0, 0, 32, 8, -14, -4, -28, -52][index]}px`,
              }}
              type="button"
            >
              <span aria-hidden="true">{recipe.model}<small>{costLabel}</small></span>
              <i />
            </button>
          )
        })}
